...
let chart = null
let cachedData = null  // 缓存数据，主题切换时直接重渲染
...
const renderChart = (data) => {
  ...
  chart.setOption({
    tooltip: {
      ...
      formatter: p => `<b style="color:${t['--accent-secondary']}">${p.name}</b><br/>数量: ${p.value} 本 (${p.percent}%)`
    },
    legend: {
    ...
    color: palette,
    series: [{
      type: 'pie', radius: ['40%', '70%'], center: ['35%', '52%'],
      avoidLabelOverlap: true,
      itemStyle: { borderRadius: 6, borderColor: t['--bg-primary'], borderWidth: 2 },
      label: { show: true, position: 'outside', formatter: '{b}\n{d}%', fontSize: 13, color: t.chartText, lineHeight: 14 },
      labelLine: { length: 12, length2: 8, lineStyle: { color: t.chartAxis } },
      emphasis: {
        label: { show: true, fontSize: 15, fontWeight: 700, color: t['--accent-secondary'] },
    ...
    graphic: [{
      ...
      style: { text: total + '\n本', fill: t.chartText, fontSize: 18, textAlign: 'center', lineHeight: 24 }
    }],
