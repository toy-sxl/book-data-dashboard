...
const renderChart = (data) => {
  ...
  chart.setOption({
    tooltip: {
      ...
      formatter: p => `<b style="color:#00d8ff">${data[p[0].dataIndex][0]}</b><br/>图书数量: ${p[0].value} 本`
    },
    grid: { left: 8, right: 12, top: 16, bottom: 8, containLabel: true },
    xAxis: {
      type: 'category', data: names,
      axisLabel: { fontSize: 12, color: 'rgba(203,213,225,0.5)', interval: 0, rotate: 25 },
    ...
    series: [{
      ...
      label: { show: true, position: 'top', fontSize: 12, color: 'rgba(203,213,225,0.6)' }
    }],
