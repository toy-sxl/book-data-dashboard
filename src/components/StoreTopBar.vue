...
const renderChart = (data) => {
  ...
  const names = data.map(([n]) => n.length > 6 ? n.slice(0, 6) + '…' : n)
  ...
  chart.setOption({
    tooltip: {
      ...
      formatter: p => `<b style="color:${t['--accent-secondary']}">${data[p[0].dataIndex][0]}</b><br/>图书数量: ${p[0].value} 本`
    },
    grid: { left: 8, right: 12, top: 16, bottom: 8, containLabel: true },
    xAxis: {
      type: 'category', data: names,
      axisLabel: { fontSize: 12, color: t.chartText, interval: 0, rotate: 25 },
    ...
    series: [{
      ...
      itemStyle: {
        ...
        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
          { offset: 0, color: t['--accent-secondary'] },
          { offset: 1, color: 'rgba(76,155,253,0.2)' }
        ])
      },
      emphasis: {
        itemStyle: {
          ...
          shadowBlur: 14, shadowColor: 'rgba(0,216,255,0.4)'
        }
      },
      label: { show: true, position: 'top', fontSize: 12, color: t.chartText }
    }],
